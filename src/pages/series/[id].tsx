...
import { getDataById, getSeries } from '@services/api'
      ...
      >
        ...
        >
          {data.result.creators.available
            ...
            : 'No Creators'}
          <span> · {data.result.startYear}</span>
        </Typography>
...
export const getStaticPaths: GetStaticPaths = async () => {
  const { data } = await getSeries(5, '-startYear')
...
export const getStaticProps: GetStaticProps = async ctx => {
  ...
  const data = await getDataById(id, 'series')
