...
import { getDataById, getComics } from '@services/api'
      ...
      >
        ...
        >
          {data.result.creators.available
            ...
            : 'No Creators'}
          <span> · {new Date(data.result.dates[0].date).getFullYear()}</span>
        </Typography>
        ...
        >
          {data.result.description || 'No description available for this comic'}
        </Typography>
      </ProfileBox>
...
export const getStaticPaths: GetStaticPaths = async () => {
  const { data } = await getComics(5, '-focDate')
...
export const getStaticProps: GetStaticProps = async ctx => {
  ...
  const data = await getDataById(id, 'comics')
